...
from src.utils.locate import _locate
...
class ConvertMode(Enum):
    ...
    NONE = "none"
    # Convert the OmegaConf config to primitive container, Structured Configs are preserved  # noqa: E501
    PARTIAL = "partial"
    # Convert the OmegaConf config to primitive container, Structured Configs are converted to  # noqa: E501
    # dataclass / attr class instances.
    OBJECT = "object"
    # Fully convert the OmegaConf config to primitive containers (dict, list and primitives).  # noqa: E501
    ALL = "all"
...
def _extract_pos_args(input_args: Any, kwargs: Any) -> Tuple[Any, Any]:
    ...
    if isinstance(config_args, Sequence):
        ...
        raise Exception(
            f"Unsupported _args_ type: '{type(config_args).__name__}'. value: '{config_args}'"  # noqa: E501
        )
...
def _call_target(
    ...
    try:
        ...
        msg = (
            f"Error in collecting args and kwargs for '{_convert_target_to_string(_target_)}':"  # noqa: E501
            + f"\n{repr(e)}"
        )
        if full_key:
            msg += f"\nfull_key: {full_key}"
    ...
    if _partial_:
        try:
            ...
            msg = (
                f"Error in creating partial({_convert_target_to_string(_target_)}, ...) object:"  # noqa: E501
                + f"\n{repr(e)}"
            )
            if full_key:
                msg += f"\nfull_key: {full_key}"
...
def _resolve_target(
    ...
    if isinstance(target, str):
        try:
            ...
            msg = f"Error locating target '{target}', set env var HYDRA_FULL_ERROR=1 to see chained exception."  # noqa: E501
            if full_key:
                msg += f"\nfull_key: {full_key}"
            raise Exception(msg) from e
    if not callable(target):
        msg = f"Expected a callable target, got '{target}' of type '{type(target).__name__}'"  # noqa: E501
        if full_key:
            msg += f"\nfull_key: {full_key}"
        raise Exception(msg)
    return target
...
def instantiate(config: Any, *args: Any, **kwargs: Any) -> Any:
    ...
    if OmegaConf.is_dict(config):
        ...
        if _partial_:
            raise Exception(
                "The _partial_ keyword is not compatible with top-level list instantiation"  # noqa: E501
            )
...
def instantiate_node(
    ...
    *args: Any,
    ...
    if OmegaConf.is_dict(node):
        # using getitem instead of get(key, default) because OmegaConf will raise an exception  # noqa: E501
